-- Roblox Fly Mode Script with Mobile Joystick Support & Expandable UI
-- Author: 170F Team
-- Features: Fly Mode (Mobile Joystick), Modern UI, Draggable, Expandable, Rainbow Text
...
controlsLabel.Size = UDim2.new(1, 0, 0, 90)
...
controlsLabel.Text = "📋 MOBILE CONTROLS\n\n🕹️ Joystick - Move\n⬆️ Jump Button - Ascend\n⬇️ Crouch Button - Descend\n🎮 Camera - Look Around"
...
local expandedSize = UDim2.new(0, 320, 0, 580)
...
-- Input Handling untuk Jump dan Crouch
...
RunService.RenderStepped:Connect(function()
    ...
    -- Flying dengan Mobile Joystick
    if flyingActive and bodyVelocity and bodyGyro then
        local camera = workspace.CurrentCamera
        
        -- Get joystick movement dari Humanoid.MoveDirection (bawaan Roblox mobile)
        local moveDirection = humanoid.MoveDirection
        
        -- Konversi joystick input ke world space
        local cameraForward = camera.CFrame.LookVector
        local cameraSide = camera.CFrame.RightVector
        
        -- Hitung movement dari joystick
        local horizontalMovement = (moveDirection.X * cameraSide + moveDirection.Z * cameraForward) * (flySpeed / 10)
        
        -- Hitung vertical movement
        local verticalMovement = Vector3.new(0, 0, 0)
        if jumpPressed then
            verticalMovement = Vector3.new(0, flyUpSpeed / 10, 0)
        elseif crouchPressed then
            verticalMovement = Vector3.new(0, -flyUpSpeed / 10, 0)
        end
        
        -- Combine semua movement
        local finalVelocity = horizontalMovement + verticalMovement
...
print("✈️ Mobile Joystick Support Enabled!")
print("📱 Use Joystick to Move, Jump to Ascend, Crouch to Descend")
